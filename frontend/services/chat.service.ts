...
export const chatService = {
  ...
  async createConversation(userId: string): Promise<ConversationListItem> {
    const res = await apiClient.post<ConversationListItem>('/conversations', { user_id: userId });
    return res.data;
  },

  async getMessages(conversationId: string, cursor?: string | null, limit = 30): Promise<PaginatedMessagesResponse> {
    const res = await apiClient.get<PaginatedMessagesResponse>(`/conversations/${conversationId}/messages`, {
      params: { cursor, limit },
    });
    return res.data;
  },
  ...
  async sendMessage(conversationId: string, content: string): Promise<MessageItem> {
    const res = await apiClient.post<MessageItem>(`/conversations/${conversationId}/messages`, { content });
    return res.data;
  },
};
