...
| Theme                  | Dual light/dark mode via `next-themes`. Ping-Me uses restrained premium glassmorphism with a predominantly monochromatic visual system. Light mode is primarily white and dark mode is primarily black/charcoal. Color should be used sparingly and intentionally. Avoid random saturated colors, neon gradients, excessive glow, and decorative effects. |
...
## Auth & Authentication Design System

- **Monochromatic Visual Foundation & Theme Architecture:** Ping-Me uses semantic CSS theme tokens as the source of truth for light and dark mode. Components should avoid unnecessary hardcoded theme-specific colors and should use the shared design system. Light mode is primarily white and off-white; dark mode is primarily black and charcoal. Color is used sparingly and intentionally. Avoid random saturated colors, neon gradients, excessive glow, and decorative effects.
- **Split Product / Auth Composition:** Desktop auth pages use a split layout with product storytelling on the left and form on the right. On mobile, the composition stacks vertically prioritizing authentication accessibility.
- **Rotating Product Messaging:** The left product panel features automated, subtle text rotation (`ProductMessageCarousel`) with `prefers-reduced-motion` support.
- **OAuth Entry Point Abstraction:** Google OAuth is prepared via `authService.loginWithGoogle()` service abstraction without faking backend authentication or fabricating tokens until backend support is added.
